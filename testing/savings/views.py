...
def savings_dashboard(request):
    ...
    for goal in page_obj:
        ...
        # Deadline in current month → probability unavailable
...
def goal_form(request, id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            with transaction.atomic():  # everything inside here is atomic
                if goal and new_goal.target_amount < goal.current_amount:
                    refund = goal.current_amount - new_goal.target_amount

                    goal.current_amount = new_goal.target_amount
                    goal.target_amount = new_goal.target_amount
                    goal.save()

                    tracker, _ = SurplusTracker.objects.get_or_create(user=request.user)
                    tracker.last_surplus += refund
                    tracker.save()
                    messages.success(
                        request,
                        f"Goal updated. Excess amount of {refund} refunded to accumulated balance."
                    )
                else:
                    new_goal.save()
