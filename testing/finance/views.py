...
def upload_income_csv(request):
    ...
    try:
        ...
        if any("debit" in f.lower() or "credit" in f.lower() or "(dr" in f.lower() or "(cr" in f.lower() for f in reader.fieldnames):
            messages.warning(
                request,
                "⚠️ This looks like a full bank statement. Please upload it in the Bank Statement section instead."
            )
...
def upload_bank_statement(request):
    ...
    - Handles Indian & international CSVs (ICICI, HDFC, SBI, Chase, Barclays, etc.)
    - Auto-detects income/expense based on text, debit/credit, +/-, Dr/Cr columns
    - Automatically classifies transactions into categories
    - Prevents expenses exceeding total income
    """
    if request.method != "POST":
        return redirect("dashboard")
    ...
    # ✅ 1.5MB safety limit
    if not csv_file or csv_file.size > 1572864:
        messages.error(request, "Please upload a valid CSV under 1.5 MB.")
        return redirect("dashboard")
    ...
    try:
        ...
        if not reader.fieldnames:
            messages.error(request, "CSV file has no headers or is invalid.")
            return redirect("dashboard")
        ...
        if not date_field or not (debit_field or credit_field or amount_field):
            messages.error(request, "This doesn't look like a valid bank statement.")
            return redirect("dashboard")

        # Pre-calculate totals for expense check
        total_income = Income.objects.filter(user=request.user).aggregate(total=Sum("amount"))["total"] or Decimal("0")
        total_expense = Expense.objects.filter(user=request.user).aggregate(total=Sum("amount"))["total"] or Decimal("0")
        ...
        for row in reader:
            ...
            # 📝 Extract the most relevant descriptive column
            desc_columns = ['source', 'name', 'description', 'narration', 'transaction details', 'memo']
            description = None
            for col in desc_columns:
                col_field = field_map.get(col)
                if col_field and row.get(col_field):
                    description = str(row[col_field]).strip()
                    break
            if not description:
            ...
            if debit_field and row.get(debit_field):
                ...
                amt_val = clean_amt(row[amount_field])
                if "-" in str(row[amount_field]) or "dr" in description.lower() or "debit" in description.lower():
                    txn_type = "expense"
                elif "+" in str(row[amount_field]) or "cr" in description.lower() or "credit" in description.lower():
                    txn_type = "income"
                else:
                    if any(w in description.lower() for w in ["salary", "refund", "interest", "deposit", "credit", "bonus"]):
                        txn_type = "income"
                    elif any(w in description.lower() for w in ["payment", "upi", "atm", "pos", "transfer", "dr", "bill", "debit"]):
                        txn_type = "expense"
                amount = amt_val
            ...
            # 🏷️ Predict category and create transaction
            if txn_type == "income":
                ...
                if (total_expense + amount) > total_income:
                    skipped += 1
                    messages.warning(request, f"Skipping '{description}' - exceeds total income!")
                    continue
        ...
        messages.success(
            ...
            f"Income: {imported_income}, Expense: {imported_expense}, Skipped: {skipped}"
        )
